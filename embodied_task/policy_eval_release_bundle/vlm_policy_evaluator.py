...
If a submission keeps suffixes in video names, glob templates are supported:
...
def resolve_policy_video(submission_root: Path, policy_template: str, model: str, item: dict[str, Any]) -> Path:
    ...
    if any(char in rendered for char in "*?["):
        ...
        matches = sorted(submission_root.glob(rendered))
        return matches[0] if matches else submission_root / rendered
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--policy-template", default=DEFAULT_POLICY_TEMPLATE, help="Policy video path template relative to --submission-root. Variables: {model}, {index}, {task}, {episode}, {episode_number}. Glob characters such as * are supported.")
