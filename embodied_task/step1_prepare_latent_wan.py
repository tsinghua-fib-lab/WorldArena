...
class TextVideoDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        for item in meta_list:
            ...
            try:
                ...
                if states.ndim != 2 or states.shape[0] != actions.shape[0]:
                    raise ValueError(
                        f"States/actions length mismatch: {states.shape} vs {actions.shape}"
                    )
        ...
        self.chunks = self.chunks[:]  # size use
        print(f"✅ Loaded {len(self.episodes)} episodes → {len(self.chunks)} chunks")

    def load_frames_using_imageio(
        self, file_path, max_num_frames, start_frame_id, interval, num_frames
    ):
        ...
        if self.is_i2v:
            return frames, first_frame_pil  # npy
        else:
            return frames
    ...
    def __getitem__(self, idx):
        ...
        #
        # print(f"self.chunk_frames: {self.chunk_frames}")
        ...
        # print(f"[DEBUG] len(video): {len(video)}, sampled_states.shape: {sampled_states.shape}, sampled_actions.shape: {sampled_actions.shape}")
        #
...
class TensorDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_path, max_samples=None):
        ...
        for task_dir in self.dataset_path.iterdir():
            ...
            #
            demo_dir = task_dir / "aloha-agilex_clean_50" / "aloha-agilex_clean_50"
            predata_dir = demo_dir / "predata_vidar"  #
        ...
        assert len(self.tensor_paths) > 0, (
            f"No '_vidar.tensors.pth' files found under {dataset_path}!"
        )
        ...
        for _ in range(5):
            try:
                ...
                if data["actions"].shape[1] != 45:
                    raise ValueError(
                        f"actions length is {data['actions'].shape[1]}, expected 45"
                    )
                clean_data = {}
                clean_data["z"] = [t.clone().detach() for t in data["z"]]  #
                clean_data["context"] = [
                    pad_tensor_to_fixed_size(emb).contiguous().clone().detach()
                    for emb in data["context"]
                ]
                clean_data["context_null"] = [
                    emb.contiguous().clone().detach() for emb in data["context_null"]
                ]
...
class LightningModelForDataProcess(pl.LightningModule):
    def __init__(self, cfg, root_path=None, size="832x480"):
        ...
        assert hasattr(self.model, "TVP_encoder"), (
            "Model must have TVP_encoder attribute"
        )
    ...
    def test_step(self, batch, batch_idx):
        ...
        # check if output file exists and has "noise" key
        if output_file.exists():
            # if "noise" in data_dict:
            print(f"✅  existing (has 'noise'): {output_file}")
            # return
            # else:
            #    print(f"⚠️ File exists but missing 'noise', will reprocess: {output_file}")

        print(
            f"Processing: {text} | First frame type: {type(first_frame)} | Output: {output_file}"
        )
        ...
        with torch.no_grad():
            prep_result = self.model.TVP_encoder._preprocess_for_i2v(
                ...
                max_area=max_area,
                frame_num=45,  # video frames num
                n_prompt="",  #
                seed=-1,
                offload_model=False,  #
            )
        print(
            f"actions shape: {actions.shape}seq_len: {prep_result['seq_len']}, frame_num: {prep_result['frame_num']}"
        )
        #
        data = {
